...
# comment traiter les observateurs

#SPECIES = "Merle leucomêle"
#SPECIES = "Batara rayé"
#SPECIES  = "Alapi à tête noire"
#SPECIES  = "Tyranneau souris"
#SPECIES  = "Viréo chivi"
#SPECIES  = "Tyran mélancolique"
SPECIES  = "Grallaire grand-beffroi"
...
depoc = subset(depoc, DURATION >= 4 ) 

depoc_sp = subset(depoc, NAME_SPECIES == SPECIES ) 


ID_FORM_sp =  tibble(ID_FORM = unique(depoc_sp$ID_FORM))
ID_FORM_all =  tibble(ID_FORM = unique(depoc$ID_FORM))
ID_FORM_non_sp = subset(ID_FORM_all, !(ID_FORM %in% ID_FORM_sp$ID_FORM))
...
emme = emmeans(mod, specs = pairwise ~ MONTH)
emme
emmip(mod, ~ MONTH, CIs = TRUE, type = "response") +
  ggtitle(SPECIES)
